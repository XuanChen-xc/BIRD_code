...
class Policy(nn.Module):
    def __init__(self, obs_shape, action_space, base=None, base_kwargs=None, beta=False, epsilon=0, device=None):
    ...
    def get_log_prob(self, inputs, rnn_hxs, masks, action):
        _, actor_features, _ = self.base(inputs, rnn_hxs, masks)
        dist = self.dist(actor_features)
        return dist.log_probs(action)
    ...
    def evaluate_actions(self, inputs, rnn_hxs, masks, action, beta=False):
        ...
        dist = self.dist(actor_features)
        self.fix_dist = dist
        self.action_features = actor_features
    ...
    def get_each_conv_pre_activation(self, x):
        i=0
        output_list = []
        for layer in self.base.main:
            x = layer(x)
            if (i%2) == 0:
...
class MyCNNBase(NNBase):
    def __init__(self, num_inputs, recurrent=False, hidden_size=512, dim=84):
        ...
        self.main = nn.Sequential(
            (MultiConv2d(num_inputs, 32, 8, stride=4)), nn.ReLU(),
            (MultiConv2d(32, 64, 4, stride=2)), nn.ReLU(),
            (MultiConv2d(64, 32, 3, stride=1)), nn.ReLU(), Flatten(),
...
class MultiConv2d(nn.Module):
    def __init__(self, num_inputs, num_outputs, kernel_size, stride):
        super(MultiConv2d, self).__init__()
        self.layers = nn.ModuleList()
        for i in range(num_outputs):
            self.layers.append(nn.Conv2d(num_inputs, 1, kernel_size, stride))
