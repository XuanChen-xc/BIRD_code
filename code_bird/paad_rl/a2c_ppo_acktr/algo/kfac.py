...
from paad_rl.a2c_ppo_acktr.utils import AddBias
    ...
    def step(self):
        ...
        for i, m in enumerate(self.modules):
            ...
            if self.steps % self.Tf == 0:
                ...
                # paad_rling with different ways to this in PyTorch.
                try:
                    self.d_a[m], self.Q_a[m] = torch.symeig(
                        self.m_aa[m], eigenvectors=True)
                except:
                    print('return eigenvalues for module fails, using np.linalg instead. Module: ', m)
                    values, vectors = LA.eigh(self.m_aa[m].detach().cpu().clone())
                    self.d_a[m] = torch.from_numpy(values).to(self.model.device)
                    self.Q_a[m] = torch.from_numpy(vectors).to(self.model.device)
                try:
                    self.d_g[m], self.Q_g[m] = torch.symeig(
                        self.m_gg[m], eigenvectors=True)
                except:
                    print('return eigenvalues for module fails, using np.linalg instead. Module: ', m)
                    values, vectors = LA.eigh(self.m_gg[m].detach().cpu().clone())
                    self.d_g[m] = torch.from_numpy(values).to(self.model.device)
                    self.Q_g[m] = torch.from_numpy(vectors).to(self.model.device)
